...
export class PokemonService {
  ...
  async findAll({limit = this.default_limit, page = 0}: PaginationDto, res: Response) {
    const pokemos_db = await this.pokemonModel.find().limit(limit).skip(page * limit).select('-__v');
